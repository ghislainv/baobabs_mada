...
packages <- c("curl", "rgdal", "sp", "raster")
...
if(!file.exists("results/environ.pdf")) {
  dir.create("results",showWarnings=FALSE)
  pdf(file="results/environ.pdf",height=10,width=10)
  plot(s)
  dev.off()
}
getwd()
...
fun.extent <- function(sp,s) {
  ...
  df.orig <- read.csv(file=paste0("data/","dados_baobas.csv"),header=TRUE,sep=",")
  ...
  sp.names <- c("Adansonia digitata","Adansonia grandidieri","Adansonia madagascariensis","Adansonia perrieri", 
              "Adansonia rubrostipa","Adansonia suarezensis","Adansonia za","Adansonia za perrieri")
##================
## Loop on species
# run.species <- function (i) {
  for (i in 1: length(sp.names)) {
     #i=7 # do this if you want to command for one specific species (according to sp.names)
  setwd("~/MEGA/Artigos/baobas_article/")
...
for (j in 1:length(rcp)) {
  for (l in 1:length(yr)) {
    ...
    # fd
    if (SDA.fut$area.fut[SDA.fut$rcp==rcp[j] & SDA.fut$yr==yr[l] & SDA.fut$disp=="full"] !=0) {
      Seas.fut$mean.fut[SDA.fut$rcp==rcp[j] & SDA.fut$yr==yr[l] & SDA.fut$disp=="full"] <- mean(values(s$tseas)[values(caFut)>1500],na.rm=TRUE)
      Seas.fut$q1.fut[SDA.fut$rcp==rcp[j] & SDA.fut$yr==yr[l] & SDA.fut$disp=="full"] <- quantile(values(s$tseas)[values(caFut)>1500],0.025,na.rm=TRUE)
      Seas.fut$q2.fut[SDA.fut$rcp==rcp[j] & SDA.fut$yr==yr[l] & SDA.fut$disp=="full"] <- quantile(values(s$tseas)[values(caFut)>1500],0.975,na.rm=TRUE)
    }
    # zd
    if (SDA.fut$area.fut[SDA.fut$rcp==rcp[j] & SDA.fut$yr==yr[l] & SDA.fut$disp=="zero"] !=0) {
      Seas.fut$mean.fut[SDA.fut$rcp==rcp[j] & SDA.fut$yr==yr[l] & SDA.fut$disp=="zero"] <- mean(values(s$tseas)[values(caZD)>1500],na.rm=TRUE)
...
# Seas
Seas.fut$change <- Seas.fut$mean.fut-Seas.fut$mean.pres
