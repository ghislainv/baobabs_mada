...
cor1_2 <- read_csv("data/baobabs_raw_data/1_2_data_corrections_A_suar_mada_confusion.csv") %>%
  mutate(TreeID=paste(Species, TreeName, Lat, Long, sep="_"))
out1 <- in1_1 %>%
  filter(!is.na(Lat) & !is.na(Long)) %>%
  rename(TreeName=`Tree-Site`) %>%
  mutate(Year="2004-2015", Institute="CIRAD", Collector="Leong Pock Tsy Jean-Michel; Danthu Pascal",
         Source="field observation", Dataset="dataset1") %>%
  # Correct two points which are far from the coast line
  mutate(Lat=ifelse(paste(Species, TreeName, sep="_") == "A_grandidieri_AdBefa_97", -22.069, Lat)) %>%
  mutate(Lat=ifelse(paste(Species, TreeName, sep="_") == "A_suarezensis_ANDAC8", -12.3283833333333, Lat)) %>%
  # Correct misidentified A_suarezensis trees
  mutate(TreeID=paste(Species, TreeName, Lat, Long, sep="_")) %>%
  mutate(Species=ifelse(TreeID %in% cor1_2$TreeID, "A_madagascariensis", Species)) %>%
  # Select columns
  select(Species, TreeName, Lat, Long, Year, Institute, Collector, Source, Dataset)
...
out2 <- in2 %>%
  rename(Lat=Y, Long=X) %>%
  mutate(Species="A_suarezensis", Year="2014", Institute="CIRAD",
...
out3 <- in3 %>%
  rename(Lat=Y, Long=X) %>%
  mutate(Species="A_grandidieri", Year="2009", Institute="CIRAD",
...
out4 <- in4 %>%
  rename(Lat=Y, Long=X) %>%
  mutate(Species="A_suarezensis", Year="2010", Institute="CIRAD",
         Source="photo-interpretation of satellite images",
         Dataset="dataset4",
...
out6 <- in6 %>%
  ...
  mutate(Lat_ddmm=paste0(substr(Position,2,3),".",
                         substr(Position,5,6),
                         substr(Position,8,10)),
         Long_ddmm=paste0(substr(Position,13,14),".",
                          substr(Position,16,17),
                          substr(Position,19,21))) %>%
  mutate(Lat=ddmm2latlong(Lat_ddmm, coord="Lat"),
         Long=ddmm2latlong(Long_ddmm, coord="Long")) %>%
  # Get species name when possible
  mutate(SpInitials=substr(Name,1,3)) %>%
  mutate(Species=ifelse(Mission %in% c("AZA_Betioky_2013","Betioky_2013"), "A_za", NA)) %>%
  mutate(Species=ifelse((Mission == "Betioky_2013") & grepl("RUBT",Name), "A_rubrostipa", Species)) %>%
  mutate(Species=ifelse(Mission == "ZAPER_2013", "A_za_perrieri", Species)) %>%
  mutate(Species=ifelse(SpInitials == "AM ", "A_madagascariensis", Species)) %>%
  mutate(Species=ifelse(SpInitials == "AR ", "A_rubrostipa", Species)) %>%
  mutate(Species=ifelse(SpInitials == "AS ", "A_suarezensis", Species)) %>%
  mutate(Species=ifelse(SpInitials == "AZ ", "A_za", Species)) %>%
  mutate(Species=ifelse(grepl("CITER", Name), "A_za", Species)) %>%
  # We remove observations for which we don't have the species name
  filter(!is.na(Species)) %>%
  # Retrieve the year
  mutate(Year=paste0(20,substr(Description,8,9))) %>%
  # Additional info
  mutate(Institute="CIRAD",
         Source="field observation",
         Dataset="dataset6",
...
out7 <- in7 %>%
  ...
  mutate(Lat_ddmm=paste0(substr(Position,2,3),".",
         ...
         Long_ddmm=paste0(substr(Position,13,14),".",
                          substr(Position,16,17),
                          substr(Position,19,21))) %>%
  mutate(Lat_calc=ddmm2latlong(Lat_ddmm, coord="Lat"),
         Long_calc=ddmm2latlong(Long_ddmm, coord="Long")) %>%
  mutate(Lat=ifelse(is.na(Lat), Lat_calc, Lat),
         Long=ifelse(is.na(Long), Long_calc, Long)) %>%
  mutate(Species="A_perrieri", Year="2004-2015", Institute="CIRAD", TreeName=Name,
         Dataset="dataset7",
...
out8_1 <- in8_1 %>%
  mutate(Lat=Y, Long=X, Species="A_suarezensis", Year="2015", Institute="CIRAD",
...
out8_2 <- in8_2 %>%
  ...
  mutate(Species=ifelse(Name=="A mada", "A_madagascariensis", Species)) %>%
  select(Species, TreeName, Lat, Long, Year, Institute, Collector, Source, Dataset)
...
out9 <- in9 %>%
  mutate(Year="2015", Institute="CIRAD", Dataset="dataset9",
